...
export async function getAllPendingReports(): Promise<PendingReport[]> {
  return getDb().getAllAsync<PendingReport>(`SELECT * FROM pending_reports WHERE status = 'pending' ORDER BY created_at ASC;`);
}
...
export function cacheMachines(machines: Array<{ id: string; data: string }>): void {
  ...
  db.withTransactionSync(() => {
    db.runSync(`DELETE FROM machine_cache;`);
    const statement = db.prepareSync(
      `INSERT OR REPLACE INTO machine_cache (id, data, updated_at) VALUES ($id, $data, $updated_at);`
    );
    for (const machine of machines) {
      statement.executeSync({ $id: machine.id, $data: machine.data, $updated_at: now });
    }
...
export async function getCachedMachines(): Promise<Array<{ id: string; data: string }>> {
  return getDb().getAllAsync<{id: string; data: string}>(`SELECT * FROM machine_cache ORDER BY updated_at DESC;`);
}
